...
import { getJournalDetails, getJournals, getReportDetails, getReports } from '../../services/graphql';
                        ...
                        <div className='space-y-5 flex items-center justify-start flex-col w-full mt-1 mb-6'>
                            <div className='flex items-center w-full justify-start gap-x-4'>
                                <div className='flex items-center justify-start w-full gap-x-4'>
                                    <Link href={`/categories/${post.categories.slice(0, 1).map((category: any) => category.slug)}`}>
                                        <span className='px-2.5 py-1 cursor-pointer text-sm font-medium rounded-md text-purple-800 bg-purple-100 hover:bg-purple-800 hover:text-white'>
                                            {post.categories.slice(0, 1).map((category: any) => category.title)}
                                        </span>
                                    </Link>
                                    <Link href={`/categories/${post.categories.slice(1, 2).map((category: any) => category.slug)}`}>
                                        <span className='px-2.5 py-1 cursor-pointer text-sm font-medium rounded-md text-purple-800 bg-purple-100 hover:bg-purple-800 hover:text-white'>
                                            {post.categories.slice(1, 2).map((category: any) => category.title)}
                                        </span>
                                    </Link>
                                </div>
                            </div>
                                    ...
                                    {info && (
                                        <motion.span
                                            ...
                                            <CopyToClipboard text={value} onCopy={() => setCopied(true)} >
                                                <motion.button onClick={toggleInfo} variants={itemVariants} onCopy={() => setCopied(true)} className='flex items-center justify-start w-full text-sm text-white mx-2 px-3 py-2 font-normal gap-x-4 cursor-pointer hover:bg-gray-800 duration-300'>
                                                    {copied ?
...
export async function getStaticProps({ params }: any) {
    const data = await getJournalDetails(params.slug);
...
export async function getStaticPaths() {
    ...
    return {
        paths: journals.map(({ node: { slug } }: any) => ({ params: { slug } })),
        fallback: true,
    };
}