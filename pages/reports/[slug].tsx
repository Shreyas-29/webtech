...
import { getReportDetails, getReports } from '../../services/graphql';
import { ArticleDetail, ArticleWidget, Author, Comments, Form, Loader } from '../../components';
...
function Reports({ report }: any) {
    ...
    const [count, setCount] = useState(report.like);
    ...
    return (
        <div className='mx-auto container'>
            <Head>
                <title>Webtech | {report.title} </title>
                <link rel="icon" href="/favicon.svg" />
            </Head>

            <header className='relative pt-16 md:py-20 lg:py-20 bg-gray-900 z-10 !w-screen top-0 left-0' key={report.id}>
                    ...
                    <div className='max-w-screen-md'>
                        <div className='space-y-5 flex items-center justify-start flex-col w-full mt-1 mb-6'>
                            <div className='flex items-center w-full justify-start gap-x-4'>
                                <div className='flex items-center justify-start w-full gap-x-4'>
                                    <Link href={`/categories/${report.categories.slice(0, 1).map((category: any) => category.slug)}`}>
                                        <span className='px-2.5 py-1 cursor-pointer text-sm font-medium rounded-md text-purple-800 bg-purple-100 hover:bg-purple-800 hover:text-white'>
                                            {report.categories.slice(0, 1).map((category: any) => category.title)}
                                        </span>
                                    </Link>
                                    <Link href={`/categories/${report.categories.slice(1, 2).map((category: any) => category.slug)}`}>
                                        <span className='px-2.5 py-1 cursor-pointer text-sm font-medium rounded-md text-purple-800 bg-purple-100 hover:bg-purple-800 hover:text-white'>
                                            {report.categories.slice(1, 2).map((category: any) => category.title)}
                                        </span>
                                    </Link>
                                </div>
                            </div>
                            <h1 className='text-3xl md:text-4xl lg:text-[3rem] lg:leading-tight max-w-4xl text-white'>
                                {report.title.substr(0, 46)}
                            </h1>
                        </div>
                        ...
                        <div className='flex flex-col sm:flex-row justify-between sm:items-end space-y-5 sm:space-y-0 sm:space-x-5 w-full py-4'>
                            <div className='flex flex-wrap items-center font-medium text-left text-sm text-gray-200 leading-none flex-shrink-0'>
                                <Link href={`/author/${report.author.slug}`}>
                                    <Image
                                        src={report.author.photo.url}
                                        alt='author'
                                        width={2000}
                                        height={2000}
                                        unoptimized
                                ...
                                <div className='ml-3'>
                                    <Link href={`/author/${report.author.slug}`}>
                                        <p className='text-sm font-semibold capitalize text-gray-50'>{report.author.name}</p>
                                    </Link>
                                    <div className='mt-1 flex items-center justify-start w-max space-x-2'>
                                        <span className='text-xs font-normal text-gray-200'>{moment(report.createdAt).format('DD MMM YYYY')}</span>
                            ...
                            <div className='flex flex-row space-x-2.5 items-center relative'>
                                ...
                                {modal && (
                                    <motion.div
                                        ...
                                        className='modal absolute transition-transform transform-gpu opacity-100 scal1\ bg-white -left-1/4 rounded-xl border p-8 shadow-xl shadow-gray-500/30 origin-center backdrop-blur-lg'>
                                        <div className='h-max max-w-md sm:w-[30rem] flex items-center justify-center relative'>
                                            <div className='flex flex-col items-center justify-start w-full'>
                                                ...
                                                <textarea name="report" id="report" className='resize-none outline-none w-full p-4 rounded-md text' rows={4} placeholder='...' />
                                                <div className='flex items-center justify-start w-full space-x-4 mt-4'>
                                                    ...
                                                    <button onClick={openModal} className='px-4 py-2 rounded-md bg-white hover:bg-gray-100 text-gray-700 border-2 font-medium text-sm'>
                                                        Cancel
                                                    </button>
                                                </div>
                                            </div>
                ...
                <div className='mt-8 md:mt-0 md:absolute md:top-0 md:right-0 md:bottom-0 md:w-1/2 lg:w-2/5 2xl:w-1/3' >
                    ...
                    <Image src={report.featuredImage.url}
                        alt='featuredImage'
                        width={2000}
                        height={2000}
                        unoptimized
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative my-10 container px-4 lg:px-36">
                <div className="col-span-1 lg:col-span-8">
                    <ArticleDetail article={report} />
                    <Author article={report} />
                    <Form slug={report.slug} />
                    <Comments slug={report.slug} />
                </div>
...
export async function getStaticProps({ params }: any) {
    const data = await getReportDetails(params.slug);
...
export async function getStaticPaths() {
    ...
    return {
        paths: reports.map(({ node: { slug } }: any) => ({ params: { slug } })),
        fallback: true,
    };
}