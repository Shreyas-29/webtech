...
import { getBlogDetails, getBlogs, getPostDetails, getPosts, getTags } from '../../services/graphql'
import { ArticleDetail, ArticleWidget, Author, CategoryWidget, Comments, Form, Loader } from '../../components';
...
function ArticleDetails({ blog }: any) {
    ...
    const [count, setCount] = useState(blog.like);
    ...
    return (
        <div className='mx-auto container'>
            <Head>
                <title>Webtech | {blog.title} </title>
                <link rel="icon" href="/favicon.svg" />
            </Head>

            <header className='relative pt-16 md:py-20 lg:py-20 bg-gray-900 z-10 !w-screen top-0 left-0' key={blog.id}>
                    ...
                    <div className='max-w-screen-md'>
                        <div className='space-y-5 flex items-center justify-start flex-col w-full mt-1 mb-6'>
                            <div className='flex items-center w-full justify-start gap-x-4'>
                                <Link href={`/categories/${blog.categories.map((category: any) => category.slug)}`}>
                                    <span className='px-2.5 py-1 cursor-pointer text-sm font-medium rounded-md text-purple-800 bg-purple-100 hover:bg-purple-800 hover:text-white'>
                                        {blog.categories.map((category: any) => category.title)}
                                    </span>
                                </Link>
                            </div>
                        ...
                        <div className='flex flex-col sm:flex-row justify-between sm:items-end space-y-5 sm:space-y-0 sm:space-x-5 w-full py-4'>
                            <div className='flex flex-wrap items-center font-medium text-left text-sm text-gray-200 leading-none flex-shrink-0'>
                                ...
                                <div className='ml-3'>
                                    <Link href={`/author/${blog.author.slug}`}>
                                        <p className='text-sm font-semibold capitalize text-gray-50'>{blog.author.name}</p>
                                    </Link>
                                    <div className='mt-1 flex items-center justify-start w-max space-x-2'>
                                        <span className='text-xs font-normal text-gray-200'>{moment(blog.createdAt).format('DD MMM YYYY')}</span>
...
export async function getStaticPaths() {
    ...
    return {
        paths: blogs.map(({ node: { slug } }: any) => ({ params: { slug } })),
        fallback: true,
    };
}