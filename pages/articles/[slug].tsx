...
import { getPostDetails, getPosts, getTags } from '../../services/graphql'
...
function ArticleDetails({ article, tag }: any) {
    ...
    const [count, setCount] = useState(article.like);
    const [value, setValue] = useState(copyUrl);
    const [copied, setCopied] = useState(false);
    const [modal, setModal] = useState(false);

    const tags = tag.map((a: any) => a.title);
    ...
    return (
        <div className='mx-auto container'>
            ...
            <header className='relative pt-16 md:py-20 lg:py-20 bg-gray-900 z-10 !w-screen top-0 left-0' key={article.id}>
                    ...
                    <div className='max-w-screen-md'>
                        <div className='space-y-5 flex items-center justify-start flex-col w-full mt-1 mb-6'>
                            <div className='flex items-center w-full justify-start gap-x-4'>
                                <Link href={`/categories/${article.categories.map((category: any) => category.slug)}`}>
                        ...
                        <div className='flex flex-col sm:flex-row justify-between sm:items-end space-y-5 sm:space-y-0 sm:space-x-5 w-full py-4'>
                            <div className='flex flex-wrap items-center font-medium text-left text-sm text-gray-200 leading-none flex-shrink-0'>
                                ...
                                <div className='ml-3'>
                                    <Link href={`/author/${article.author.slug}`}>
                                        <p className='text-sm font-semibold capitalize text-gray-50'>{article.author.name}</p>
                                    </Link>
                                    <div className='mt-1 flex items-center justify-start w-max space-x-2'>
                                        <span className='text-xs font-normal text-gray-200'>{moment(article.createdAt).format('DD MMM YYYY')}</span>
                ...
                <div className='mt-8 md:mt-0 md:absolute md:top-0 md:right-0 md:bottom-0 md:w-1/2 lg:w-2/5 2xl:w-1/3' >
                    ...
                    <Image src={article.featuredImage.url}
                        alt='featuredImage'
                        width={2000}
                        height={2000}
                        unoptimized
...
export async function getStaticPaths() {
    ...
    return {
        paths: articles.map(({ node: { slug } }: any) => ({ params: { slug } })),
        fallback: true,
    };
}